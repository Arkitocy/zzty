    ...
    public void showReservation(Customer customer) {
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            if (customer.equals(reservation.getCar().getCustomer())) {
                System.out.println("顾客" + reservation.getCar().getCustomer().getName() + "的车" + reservation.getCar().getCarId() + "预约在" + reservation.getReservationDate()+"服务。预约时间："+reservation.getSetDate());
            }
        }
    }
